...
function FieldRow({ name, value }: { name: string; value: unknown }) {
  ...
  return (
    <div className="flex items-baseline gap-[8px]">
      <span className="chrome flex-none text-[8.5px] tracking-[0.1em] text-ink-dim uppercase">
        {name}
      </span>
...
function TrackerBlock({ tracker, sceneId }: { tracker: TrackerDto; sceneId: string }) {
  ...
  return (
    <div className="border-b border-rule py-[8px]">
      <div className="flex items-baseline gap-[8px]">
        <span className="min-w-0 flex-1 truncate text-[11px] font-medium uppercase tracking-[0.06em]">
          {title}
        </span>
        <span className="chrome flex-none text-[8.5px] text-ink-muted uppercase">
          {tracker.tokenCount} TOK{tracker.isPinned ? " · PINNED" : ""}
        </span>
        <button
          type="button"
          className="chrome flex-none text-[8.5px] uppercase"
          onClick={() => {
            setEditing(!editing);
      ...
      {editing ? (
        <textarea
          className="field chrome mt-[6px] min-h-[80px] resize-y text-[11px] leading-[1.5]"
          value={draft}
        ...
        <p className="chrome mt-[4px] text-[10px] text-ink-dim">{tracker.content}</p>
      ) : (
              ...
              <div key={key} className="space-y-[1px]">
                {(value as Record<string, unknown>[]).map((member, index) => (
                  <div key={index} className="pl-[8px]">
                    <span className="chrome text-[10px] text-ink-label">
    ...
    <div className="flex-none border-t border-rule bg-bg-raised">
      <div className="mx-auto w-full max-w-[var(--onsen-prose-measure)] px-[16px] py-[6px]">
        ...
        >
          ...
          <span className="chrome text-[10px] text-ink-muted">{open ? "▾" : "▸"}</span>
        </button>
        ...
        {open ? (
          <div className="pt-[4px]">
            ...
            <button
              type="button"
              className="chrome mt-[8px] text-[8.5px] tracking-[0.1em] text-ink-dim uppercase"
