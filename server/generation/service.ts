...
export class GenerationService {
   ...
   * scenario, and a narrator opening, generated by the model and applied in
   * one place so the three agree. The opening lands as a narrator turn, so the
   * scene opens on the setup rather than an empty room.
   */
  public async describeScene(
    sceneId: number,
    premise: string,
  ): Promise<
    | { ok: true; title: string; scenario: string; opening: string }
    | { ok: false; error: string }
  > {
    const scene = findSceneById(this.db, sceneId);
    if (scene === null) return { ok: false, error: "No such scene." };

    const kind: SideCallOp = {
      key: "scene_setup",
      runs: "side_call",
      label: "Scene setup",
      description: "Titles the scene and writes its framing and opening from the reader's premise.",
      stage: "sidecar",
    ...
    const outcome = await this.tasks.run({
      kind,
      sceneId,
      prompt: buildSideCallPrompt(
        sceneSetupPrompt(premise),
    ...
    const parsed = parseSceneSetup(outcome.text);
    if (parsed === null) return { ok: false, error: "The model did not answer in the expected shape." };

    this.db
      .query(
        "UPDATE scenes SET title = $title, scenario_override = $scenario, updated_at = $now WHERE id = $id",
      )
      .run({ title: parsed.title, scenario: parsed.scenario, now: Date.now(), id: sceneId });
...
/** The scene-setup question: the premise in, three agreed answers out (§157). */
function sceneSetupPrompt(premise: string): string {
  return [
    "The reader wants to start a roleplay and described the scene in their own words:",
    "",
    `"${premise}"`,
    "",
    "Set the scene up. Answer as JSON only, with no other text:",
    "{",
    '  "title": "a short title for the scene",',
    '  "scenario": "one or two sentences framing the scene — where, when, the situation",',
    '  "opening": "a short paragraph in the narrator\'s voice that establishes the scene and ends where the characters can act"',
    "}",
  ].join("\n");
}

/** The model's answer, parsed defensively: a JSON fence, or a bare object. */
function parseSceneSetup(
  text: string,
): { title: string; scenario: string; opening: string } | null {
  const match = text.match(/\{[\s\S]*\}/);
  if (match === null) return null;
  try {
    const parsed = JSON.parse(match[0]) as { title?: unknown; scenario?: unknown; opening?: unknown };
    if (
    ...
    if (title === "" || scenario === "" || opening === "") return null;
